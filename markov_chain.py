...
# sys.path.insert(0, '../Tweeter')
# from Tweeter.sampling import weighted_choice
...
class markov(dict):
    ...
    def generate_sentence(self, text_list, count=10):
        ...
        # add second word to tuple, then remove the head
...
def read_text_file(text):
    '''read text file and saves it to an array'''
    with open(text) as f:
        word_data = f.read()
    return word_data
    
def list_convert(text_string):
    '''Take a string and converts it to a list, then returns it.'''
